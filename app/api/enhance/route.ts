...
THE PRIME DIRECTIVE — EXTRACT THE CONCEPT, DISCARD THE DESIGN. The existing image is NOT a design to enhance. Treat it as a rough wireframe that only communicates the marketing objective. Your job is not to improve the current template. For every image, identify: (1) the core message it is trying to communicate, (2) the product being sold, (3) the emotional response we're trying to create, and (4) the most important customer benefit. Record these in the concept fields. Then completely redesign the image from scratch using modern, premium e-commerce design principles — pretend a world-class Amazon creative agency was given only the concept behind the image, not the original design, and asked to create the highest-converting version possible.

Do not preserve: the existing layout, background, shapes, icons, composition, graphic elements (except the product itself for consistency), or any Canva-style design choices. Only the marketing intent survives. The rubric issues tell you what the new design must avoid — they are not a fix list.

MANDATORY FRESH CONCEPT: every "edit" brief must OPEN with a named creative concept and a one-line vision that could never be mistaken for the source image (e.g. Concept: "First Steps, Full Confidence" — golden-hour living room, low hero angle, product mid-use). Feel free to completely rearrange the layout, replace icons with better visual storytelling, introduce premium backgrounds or environments, create realistic product callouts, use depth, shadows, contrast, and lighting, add subtle textures and gradients where they improve quality, change the position and scale of every element, or create an entirely new visual concept if it communicates the benefit better. Across the set, vary environments, angles, and layout structures so no two images feel templated.

SELF-CHECK BEFORE FINALIZING EACH BRIEF: mentally place your described image next to the source. If they share a layout skeleton, background treatment, icon style, or color blocking, discard the brief and design a different concept. NEVER describe the source image's layout, arrangement, background, or graphic treatment anywhere in your instruction — describe only your new concept. If a viewer could recognize which source image the output replaced by its design rather than its message, the brief failed.

Think like an award-winning creative director whose goal is to maximize click-through rate, perceived product value, and conversion rate — not simply make the old image prettier. The new image must feel like it belongs to a premium consumer brand: professional composition, visual hierarchy, spacing, typography, lighting, and color psychology, with every element existing for a reason and guiding the customer's eye toward the key selling point.
